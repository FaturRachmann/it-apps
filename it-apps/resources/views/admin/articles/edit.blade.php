<x-admin-layout>
    ...
    <div style="max-width: 900px;">
        ...
        <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 32px;">
            ...
            <a href="{{ route('articles.show', $article->slug) }}" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; padding: 10px 16px; background: var(--blue-50); border-radius: 10px; text-decoration: none; color: var(--blue-600); font-size: 0.9rem; font-weight: 600; transition: all 0.2s;"
               onmouseover="this.style.background='var(--blue-100)'"
               onmouseout="this.style.background='var(--blue-50)'">
        ...
        <!-- Form Card -->
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" style="background: white; border-radius: 20px; box-shadow: 0 2px 12px rgba(0,0,0,0.08); overflow: hidden;">
            @csrf
            @method('PUT')
            ...
            <div style="padding: 32px 40px; border-bottom: 1px solid var(--gray-100); background: linear-gradient(135deg, var(--gray-50) 0%, white 100%);">
                <h2 style="font-size: 1.5rem; font-weight: 800; color: var(--navy-900); margin-bottom: 8px; letter-spacing: -0.02em;">Edit Artikel</h2>
                <p style="color: var(--gray-500); font-size: 0.9rem;">Update informasi artikel blog Anda</p>
            </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 28px;">
                    ...
                    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 24px;">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required placeholder="Masukkan judul artikel"
                        ...
                        <div>
                            ...
                            <input type="text" name="category" id="category" value="{{ old('category', $article->category ?? 'Tips') }}" placeholder="Tips"
                    ...
                    <div>
                        <label for="slug" style="display: block; font-size: 0.875rem; font-weight: 700; color: var(--gray-700); margin-bottom: 10px; letter-spacing: 0.02em;">
                            Slug (URL) <span style="color: var(--gray-400); font-weight: 400; font-size: 0.8rem;">- otomatis atau custom</span>
                        </label>
                        <div style="position: relative;">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $article->slug) }}" placeholder="judul-artikel-anda"
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" id="excerpt" rows="3" required placeholder="Tulis ringkasan singkat tentang artikel ini (1-2 kalimat)"
                            ...
                            onfocus="this.style.borderColor='var(--blue-400)'; this.style.background='white'; this.style.boxShadow='0 0 0 4px rgba(59,130,246,0.1)'"
                            onblur="this.style.borderColor='var(--gray-200)'; this.style.background='var(--gray-50)'; this.style.boxShadow='none'">{{ old('excerpt', $article->excerpt) }}</textarea>
                        @error('excerpt')
                            <p style="margin-top: 8px; font-size: 0.8rem; color: #dc2626; font-weight: 500;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="15" required placeholder="Tulis konten artikel lengkap di sini..."
                            ...
                            onfocus="this.style.borderColor='var(--blue-400)'; this.style.background='white'; this.style.boxShadow='0 0 0 4px rgba(59,130,246,0.1)'"
                            onblur="this.style.borderColor='var(--gray-200)'; this.style.background='var(--gray-50)'; this.style.boxShadow='none'">{{ old('content', $article->content) }}</textarea>
                        @error('content')
                            <p style="margin-top: 8px; font-size: 0.8rem; color: #dc2626; font-weight: 500;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
                        <div>
                            ...
                            <input type="number" name="read_time" id="read_time" value="{{ old('read_time', $article->read_time ?? 5) }}" placeholder="5" min="1"
                                style="width: 100%; padding: 14px 18px; border: 2px solid var(--gray-200); border-radius: 12px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 1rem; color: var(--gray-800); transition: all 0.2s; outline: none; background: var(--gray-50);"
                                onfocus="this.style.borderColor='var(--blue-400)'; this.style.background='white'; this.style.boxShadow='0 0 0 4px rgba(59,130,246,0.1)'"
                                onblur="this.style.borderColor='var(--gray-200)'; this.style.background='var(--gray-50)'; this.style.boxShadow='none'">
                        </div>
                    ...
                    <div style="padding: 20px; background: linear-gradient(135deg, var(--gray-50) 0%, white 100%); border-radius: 12px; border: 2px solid var(--gray-200);">
                        <div style="display: flex; align-items: center; gap: 14px;">
                            ...
                            <span style="display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; border-radius: 100px; font-size: 0.75rem; font-weight: 700; {{ old('is_published', $article->is_published) ? 'background: #dcfce7; color: #16a34a;' : 'background: var(--gray-100); color: var(--gray-600);' }}">
                                <span style="width: 8px; height: 8px; border-radius: 50%; {{ old('is_published', $article->is_published) ? 'background: #16a34a;' : 'background: var(--gray-400);' }}"></span>
                                {{ old('is_published', $article->is_published) ? 'Published' : 'Draft' }}
                            </span>
                        </div>
                    </div>
